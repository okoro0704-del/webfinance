...
-- Matches subdomain, custom_domain, and www.* variants.
...
begin
  ...
  v_apex := regexp_replace(v_host, '^www\.', '');
  ...
  where coalesce(d.is_master, false) = false
    ...
    and (
      ...
      or lower(coalesce(d.custom_domain, '')) = v_apex
      or lower('www.' || coalesce(d.custom_domain, '')) = v_host
    )
  limit 1;
end;
$$;
